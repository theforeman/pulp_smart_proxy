"""Tests that sync smrt plugin repositories."""
...
from pulp_smart_proxy.tests.functional.constants import (
    SMRT_FIXTURE_SUMMARY,
    SMRT_INVALID_FIXTURE_URL,
)
from pulp_smart_proxy.tests.functional.utils import (
    gen_smrt_client,
    gen_smrt_remote,
)
from pulp_smart_proxy.tests.functional.utils import set_up_module as setUpModule  # noqa:F401

from pulpcore.client.pulp_smart_proxy import (
    RepositoriesSmrtApi,
    RepositorySyncURL,
    RemotesSmrtApi,
)
...
class BasicSyncTestCase(unittest.TestCase):
    """Sync a repository with the smrt plugin."""

    @classmethod
    def setUpClass(cls):
        """Create class-wide variables."""
        cls.cfg = config.get_config()
        cls.client = gen_smrt_client()

    def test_sync(self):
        """Sync repositories with the smrt plugin.
        ...
        8. Assert that the same number of are present and that no units were
           added.
        """
        repo_api = RepositoriesSmrtApi(self.client)
        remote_api = RemotesSmrtApi(self.client)

        repo = repo_api.create(gen_repo())
        self.addCleanup(repo_api.delete, repo.pulp_href)

        body = gen_smrt_remote()
        ...
        self.assertDictEqual(get_content_summary(repo.to_dict()), SMRT_FIXTURE_SUMMARY)
        self.assertDictEqual(get_added_content_summary(repo.to_dict()), SMRT_FIXTURE_SUMMARY)
...
class SyncInvalidTestCase(unittest.TestCase):
    ...
    @unittest.skip("FIXME: Plugin writer action required.")
    def test_invalid_smrt_content(self):
        ...
        """
        with self.assertRaises(PulpTaskError) as cm:
            task = self.do_test(SMRT_INVALID_FIXTURE_URL)
        task = cm.exception.task.to_dict()
