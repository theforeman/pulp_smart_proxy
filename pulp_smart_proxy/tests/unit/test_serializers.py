...
from pulp_smart_proxy.app.serializers import SmrtContentSerializer
from pulp_smart_proxy.app.models import SmrtContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create SmrtContent
# Provide sufficient parameters to create the SmrtContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestSmrtContentSerializer(TestCase):
    """Test SmrtContentSerializer."""

    def setUp(self):
        """Set up the SmrtContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the SmrtContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = SmrtContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the SmrtContentSerializer does not accept data."""
        SmrtContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = SmrtContentSerializer(data=data)
