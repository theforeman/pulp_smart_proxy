...
from pulp_smart_proxy.app.models import SmrtContent, SmrtRemote
...
def synchronize(remote_pk, repository_pk, mirror):
    ...
    """
    remote = SmrtRemote.objects.get(pk=remote_pk)
    ...
    first_stage = SmrtFirstStage(remote, deferred_download)
    DeclarativeVersion(first_stage, repository, mirror=mirror).create()


class SmrtFirstStage(Stage):
    """
    The first stage of a pulp_smart_proxy sync pipeline.
    """
    ...
    async def run(self):
        ...
        for entry in self.read_my_metadata_file_somehow(result.path):
            unit = SmrtContent(entry)  # make the content unit in memory-only
    ...
    def read_my_metadata_file_somehow(self, path):
        """
        Parse the metadata for smrt Content type.
